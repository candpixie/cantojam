...
上述兩項修正正是與傳統階梯相抵觸之處，以下附上 95% Wilson 信賴區間：

| 組合 | 方向 | 比率 | 樣本數 | 95% 信賴區間 |
| --- | --- | --- | --- | --- |
| 六→五 | 上行 | 96.7% | 1559 | 95.7% – 97.5% |
| 五→六 | 下行 | 92.8% | 1216 | 91.2% – 94.1% |
| 四→一 | 上行 | 97.1% | 725 | 95.6% – 98.1% |
| 四→六 | 上行 | 97.9% | 1911 | 97.2% – 98.5% |
...
`validate.py` 會以建立出來的模型反過來檢查原本的語料，這只能證明模型自身一致。
更嚴格的問題請用上文的 `evaluate.py`。真實的粵語流行曲至少理應通過自身的規則，
事實亦然：
...
語料中有 15.9% 的音節唱在多於一個音上，因而隱藏了約 6,500 個音符，
而 cantojam 一直只保留每個音節的第一個音。這本來只是權宜之計，
`scripts/melisma.py` 便是用來檢驗此舉是否穩妥。

直觀假設是：上升調（第二、第五聲）的拖腔應該上行，下降調（第四聲）應該下行。
結果並非如此：

| 所屬聲調 | 樣本數 | 上行 | 持平 | 下行 | 淨值 |
| --- | --- | --- | --- | --- | --- |
| 一 陰平 | 1683 | 0.4% | 73.5% | 26.1% | −0.68 |
| 二 陰上 | 871 | 1.1% | 75.8% | 23.1% | −0.61 |
| 三 陰去 | 1244 | 1.4% | 88.7% | 9.8% | −0.23 |
| 六 陽去 | 923 | 1.3% | 87.5% | 11.2% | −0.23 |
| 五 陽上 | 807 | 1.5% | 93.6% | 5.0% | −0.08 |
| 四 陽平 | 501 | **5.0%** | 93.4% | 1.6% | **+0.13** |

上升調反而上行得**最少**（1.3%，n=1678）；上行比率最高的，
是位置最低、本身屬下降調的第四聲（5.0%，n=501）。

真正能預測拖腔走向的，是該聲調所處的高度：聲調實測高度與拖腔淨移動之間
**r = −0.940**。高音調的拖腔只能向下走，最低的聲調只能向上走。
這是音域的天花板與地板效應，與調型無關。

因此，把拖腔音節保留在第一個音上是正確的做法；而拖腔本身可由聲調高度推算，
並非只能忽略。
...
以上數字屬描述性質，描述的是模型所依據的語料本身。真正重要的問題是規則能否推廣，
因此 `scripts/evaluate.py` 會執行**以歌曲為單位切分**的五折交叉驗證：
僅以訓練歌曲建立模型，再在留出的歌曲上評分。

以歌曲而非以音節切分十分重要。同一首歌之內的音節並不獨立，
若隨機切分，同一段旋律便會同時出現在兩邊，令所有分數虛高。

任務設定為預測：給定相鄰兩個音節及其聲調，預測旋律會上行、持平還是下行。
...
**在從未見過的歌曲上，實測模型比 0243 階梯高出 10.7 個百分點**，
而此差距遠大於各折之間的離散程度（標準差約 0.011）。

僅由訓練歌曲學得的硬性規則，在留出資料上的違規率為 **4.22%**
（各折介乎 3.51% 至 4.99%）。對照 `validate.py` 的同源資料數字 3.91%：
模型移到未見過的歌曲時只損失約三分之一個百分點，
這正是這些規則屬於整個曲種的真實性質、而非那 105 首歌的偶然產物的證據。
